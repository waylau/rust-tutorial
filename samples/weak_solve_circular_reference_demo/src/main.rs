...
* Weak通过`use std::rc::Weak`来引入
* Weak可访问，但没有所有权，不增加引用计数，因此不会影响被引用值的释放回收
* 可由`Rc<T>`调用downgrade方法转换成`Weak<T>`
* `Weak<T>`可使用upgrade方法转换成`Option<Rc<T>>`，如果资源已经被释放，则Option的值是None